    ...
    void Start()
    {
        ...
        if(activeScene == $"FloorGoal{GameManager.instance.floorIndex}")
        {
            ...
            else
            {
                ...
                // ボスを召喚する
                string enemy_name = GameManager.instance.GetRandomBossEnemy().name;
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (enemySpawnPoints.Count > 0)
                    {
                        ...
                        {
                            // GM.FloorEnemyに名前だけ追加していく
                            GameManager.instance.FloorEnemy.Add(string.Copy(GameManager.instance.GetRandomEnemy().name));
                            // バトルをしたかどうかを判定する
                            GameManager.instance.isBattledFloorEnemy.Add(true);
                        }
                        for (int i = 0; i < enemySpawnPoints.Count; i++)
