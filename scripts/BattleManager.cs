...
public class BattleManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < GameManager.instance.battleEnemies.Count; i++)
        {
            ...
            // 動的に召喚
            GameObject enemy = Instantiate(enemy_prefab, new Vector3(0.0f + (float)(i-1) * 10, 3f, 0.0f), Quaternion.identity);
            ...
            c_enemy.gameObject.transform.position = new Vector3(0.0f + (float)(i-1) * 3, 3f, 0.0f);
            ...
            switch (game_state)
            {
                ...
                case GameState.SELECT_COMMAND:
                    ...
                    else
                    {
                        ...
                        // japanese
                        messageWindow.SetMessage($"{nowCharacter.Name} は {nowCharacter.selectCmd.Name} を {nowCharacter.target.Name} にした\n");
                ...
                case GameState.RESULT:
                    ...
                    {
                        // burstModeで、かつ、メッセージ表示をしていない場合
                        if (c.isFullFrustrated && !c.isSetMessageBurstMode)
                        {
                            messageWindow.SetMessage($"一定のダメージを受け、{c.Name} は バースト状態 になった !!\n");
                            messageWindow.AddMessage($"これから {c.max_turn} ターンは MP消費 なし !!");
                            yield return new WaitUntil(() => returnkeyPressed);
                            returnkeyPressed = false;

                            // メッセージ表示のフラグを立てる
                            c.isSetMessageBurstMode = true; // GameState.ORDERステイとでもとに戻す
                        }
                            ...
                            for(int i=0; i<partyMember.Count; i++)
                            {
                                ...
                                if (GameManager.instance.isParty[i])
                                {
                                    if (i==0)
                                    {
                                        ...
                                        messageWindow.AddMessage($"HP: +50, MP: +5, 攻撃力: +10\n 守備力: +30, すばやさ: +15, 回復値: +10\n");
                                        yield return new WaitUntil(() => returnkeyPressed);
                                        returnkeyPressed = false;
                                    }
                                    else if (i==1)
                                    {
                                        ...
                                        if (!GameManager.instance.isNpcsDie[0])
                                        {
                                            ...
                                            GameManager.instance.NpcsSPD[0] += 25;
                                            GameManager.instance.NpcsREC[0] += 20;

                                            // messageWindowに表示
                                            messageWindow.SetMessage("Archer:\n");
                                            messageWindow.AddMessage($"HP: +30, MP: +10, 攻撃力: +10\n 守備力: +20, すばやさ: +25, 回復値: +20\n");
                                            yield return new WaitUntil(() => returnkeyPressed);
                                            returnkeyPressed = false;
                                        }
                                    ...
                                    else if (i==2)
                                    {
                                        ...
                                        if (!GameManager.instance.isNpcsDie[1])
                                        {
                                            ...
                                            GameManager.instance.NpcsDEF[1] += 30;
                                            GameManager.instance.NpcsSPD[1] += 3;
                                            GameManager.instance.NpcsREC[1] += 1;

                                            // messageWindowに表示
                                            messageWindow.SetMessage("Warrior:\n");
                                            messageWindow.AddMessage($"HP: +70, MP: +5, 攻撃力: +15\n 守備力: +30, すばやさ: +3, 回復値: +1\n");
                                            yield return new WaitUntil(() => returnkeyPressed);
                                            returnkeyPressed = false;
                                        }
