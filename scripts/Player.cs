...
public class Player : Character
{
    ...
    void Start()
    {
        ...
        {
            // nameが一致した時、そのキャラクタのステータスを設定する
            if (name == GameManager.instance.alies[i].name)
            {
                max_hp = GameManager.instance.nowAliesStatus[i]["max_hp"];
                hp = GameManager.instance.nowAliesStatus[i]["hp"];
                max_mp = GameManager.instance.nowAliesStatus[i]["max_mp"];
                mp = GameManager.instance.nowAliesStatus[i]["mp"]; ;
                atk = GameManager.instance.nowAliesStatus[i]["atk"];
                def = GameManager.instance.nowAliesStatus[i]["def"];
                spd = GameManager.instance.nowAliesStatus[i]["spd"];
                rec = GameManager.instance.nowAliesStatus[i]["rec"];
                // 死んでいる場合
                if (hp <= 0)
                {
                    // 死亡フラグを立てる
                    GameManager.instance.isAliesDead[i] = true;
                    // active falseにする
                ...
                {
                    // メンバーになっていたら、active trueにする
                    if (GameManager.instance.isParty[i]) transform.gameObject.SetActive(true);
