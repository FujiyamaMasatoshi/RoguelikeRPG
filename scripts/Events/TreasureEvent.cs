...
public class TreasureEvent : MonoBehaviour
{
    ...
    public void GetPartyMember()
    {
        ...
        {
            if (!GameManager.instance.isParty[i + 1])
            {
                NPCs[i].SetActive(true);
                GameManager.instance.isParty[i+1] = true; //パーティ追加のフラグ

                // ステータスセット
                // hp
                GameManager.instance.nowAliesStatus[i+1]["max_hp"] = GameManager.instance.initAliesStatus[i+1]["max_hp"];
                GameManager.instance.nowAliesStatus[i+1]["hp"] = GameManager.instance.initAliesStatus[i+1]["max_hp"];
                // mp
                GameManager.instance.nowAliesStatus[i + 1]["max_mp"] = GameManager.instance.initAliesStatus[i + 1]["max_mp"];
                GameManager.instance.nowAliesStatus[i + 1]["mp"] = GameManager.instance.initAliesStatus[i + 1]["max_mp"];
                //atk
                GameManager.instance.nowAliesStatus[i + 1]["atk"] = GameManager.instance.initAliesStatus[i + 1]["atk"];
                //def
                GameManager.instance.nowAliesStatus[i + 1]["def"] = GameManager.instance.initAliesStatus[i + 1]["def"];
                //spd
                GameManager.instance.nowAliesStatus[i + 1]["spd"] = GameManager.instance.initAliesStatus[i + 1]["spd"];
                //rec
                GameManager.instance.nowAliesStatus[i + 1]["rec"] = GameManager.instance.initAliesStatus[i + 1]["rec"];

                Character p = NPCs[i].GetComponent<Character>();
                p.max_hp = GameManager.instance.initAliesStatus[i + 1]["max_hp"];
                p.hp = GameManager.instance.initAliesStatus[i + 1]["hp"];
                p.max_mp = GameManager.instance.initAliesStatus[i + 1]["max_mp"];
                p.mp = GameManager.instance.initAliesStatus[i + 1]["mp"];
                p.atk = GameManager.instance.initAliesStatus[i + 1]["atk"];
                p.def = GameManager.instance.initAliesStatus[i + 1]["def"];
                p.spd = GameManager.instance.initAliesStatus[i + 1]["spd"];
                p.rec = GameManager.instance.initAliesStatus[i + 1]["rec"];
                GameManager.instance.isAliesDead[i] = false;

                EWindow.SetActive(false);
                MWindow.SetActive(true);
                messageWindow.SetMessage($"{NPCs[i].name} が 仲間に加わった");
                break;
            }
        }
        //// Archerを加える
    ...
    public void StatusUp()
    {
        ...
        if(randomInt == 0)
        {
            ...
            for (int i = 0; i < GameManager.instance.alies.Count; i++)
            {
                ...
                {
                    // hp GM.statusUpRateだけ上昇
                    dMaxHP = (int)(GameManager.instance.statusUpRate * (float)GameManager.instance.nowAliesStatus[i]["max_hp"] - (float)GameManager.instance.nowAliesStatus[i]["max_hp"]);
                    GameManager.instance.nowAliesStatus[i]["max_hp"] += dMaxHP;
                    GameManager.instance.nowAliesStatus[i]["hp"] += dMaxHP;
            ...
            messageWindow.SetMessage($"味方全員の HP が上がった !!");
        }
        else if(randomInt == 1)
        {
            int dMaxMP = 0;
            // MP上限を上げて、回復
            for (int i = 0; i < GameManager.instance.isParty.Count; i++)
            {
                if (GameManager.instance.isParty[i])
                {
                    dMaxMP = (int)(GameManager.instance.statusUpRate * (float)GameManager.instance.nowAliesStatus[i]["max_mp"] - (float)GameManager.instance.nowAliesStatus[i]["max_mp"]);
                    GameManager.instance.nowAliesStatus[i]["max_mp"] += dMaxMP;
                    GameManager.instance.nowAliesStatus[i]["mp"] += dMaxMP;
                    //// Thiefの時
                    //if (i == 0)
                    //{
            ...
            messageWindow.SetMessage($"味方全員の MP が 上がった !!");
        }
        else if(randomInt == 2)
        {
            // HP, MP以外のステータスを上昇させる
            for (int i = 0; i < GameManager.instance.isParty.Count; i++)
            {
                ...
                {
                    // atk
                    int dAtk = (int)(GameManager.instance.statusUpRate * (float)GameManager.instance.nowAliesStatus[i]["atk"] - (float)GameManager.instance.nowAliesStatus[i]["atk"]);
                    GameManager.instance.nowAliesStatus[i]["atk"] += dAtk;
                    // def
                    int dDef = (int)(GameManager.instance.statusUpRate * (float)GameManager.instance.nowAliesStatus[i]["def"] - (float)GameManager.instance.nowAliesStatus[i]["def"]);
                    GameManager.instance.nowAliesStatus[i]["def"] += dDef;
                    // spd
                    int dSpd = (int)(GameManager.instance.statusUpRate * (float)GameManager.instance.nowAliesStatus[i]["spd"] - (float)GameManager.instance.nowAliesStatus[i]["spd"]);
                    GameManager.instance.nowAliesStatus[i]["spd"] += dSpd;
                    // rec
                    int dRec = (int)(GameManager.instance.statusUpRate * (float)GameManager.instance.nowAliesStatus[i]["rec"] - (float)GameManager.instance.nowAliesStatus[i]["rec"]);
                    GameManager.instance.nowAliesStatus[i]["rec"] += dRec;
            ...
            messageWindow.SetMessage("味方全員の 攻撃力 守備力 素早さ 回復量 が上がった!!");
        }
